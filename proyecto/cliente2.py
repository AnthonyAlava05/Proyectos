...
class BattleshipClient:
    def __init__(self, master):
        ...
        self.server = xmlrpc.client.ServerProxy("http://192.168.100.169:9000", allow_none=True)
        self.jugador = ""
        self.nombre = ""
    ...
    def check_otros_jugador_frame(self):
        if self.server.ambos_jugadores_registrados() and not self.tablero_activo:
            self.tablero_activo = True
            self.actualizar_tablero_propio()
            for fila in self.botones_tablero_oponente:
                for boton in fila:
                    boton.config(state="normal")
            self.mensaje_label.config(text="Ambos jugadores están listos. ¡Comienza el juego!", fg="green")
        else:
            self.master.after(1000, self.check_otros_jugador_frame)

    def crear_tablero(self, frame, interactivo):
        botones = []
        for row in range(10):
            fila = []
            for col in range(10):
                boton = tk.Button(frame, text="~", width=2, height=1, bg="lightblue",
                                  state="disabled" if not interactivo else "normal",
    ...
    def actualizar_tablero_propio(self):
        ...
        for i in range(10):
            for j in range(10):
                celda = tablero[i][j]
                color = "green" if celda == self.jugador else "lightblue"
    ...
    def disparar(self, x, y):
        ...
        if resultado["resultado"] in ["victoria", "fin"]:
            self.mensaje_label.config(fg="green")
            for fila in self.botones_tablero_oponente:
                for boton in fila:
